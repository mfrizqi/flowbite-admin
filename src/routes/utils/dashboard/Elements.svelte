<script lang="ts">
	...
	import StatisticsBadge from './StatisticsBadge.svelte';

	export let dark: boolean = false;
	let elementData: any;

	const headers = ['Name', 'Atomic Number', 'Atomic Symbol', 'Units', 'Status'];
	...
	<div class="items-center justify-between lg:flex">
		<div class="mb-4 mt-px lg:mb-0">
			...
			<span class="text-base font-normal text-gray-500 dark:text-gray-400">
				A list of latest registered elements
			</span>
		</div>
		...
		<TableBody>
			{#if fetchData}
				{#each fetchData as data}
					<TableBodyRow>
						<TableBodyCell class="px-4 font-normal">{data?.chemical.name}</TableBodyCell>
